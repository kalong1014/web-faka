...
AdminSes_audit();
$bh=$_GET[bh];
while1("bh,userid","yjcode_pro where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){exit;}
$tplj1="upload/".$row1[userid]."/".$bh."/";

 if($_GET[action]=="update"){  //上传
 $sj=date("Y-m-d H:i:s");
 $nbh=time();
 $wjv=$tplj1.$nbh.".jpg";
 intotable("yjcode_clear","bh,tp,type1,sj","'".$bh."','".$wjv."','商品','".$sj."'");$clearid=mysql_insert_id();
 uploadtpnodata(1,$tplj1,$nbh.".jpg","allpic",800,0,350,350,"yes",200,200);
 php_toheader("tpupload.php?bh=".$bh."&tpbh=".$nbh."&clearid=".$clearid);
 
 }elseif($_GET[action]=="del"){ //删除
  if(panduan("bh","yjcode_pro where bh='".$bh."'")==1){
  delFile("../".$tplj1.$_GET[tpbh].".jpg");
  delFile("../".$tplj1.$_GET[tpbh]."-1.jpg");
  delFile("../".$tplj1.$_GET[tpbh]."-2.jpg");
  if($_GET[clearid]!=""){deletetable("yjcode_clear where id=".$_GET[clearid]);}
  if($_GET[tpid]!=""){deletetable("yjcode_tp where id=".$_GET[tpid]);}
  php_toheader("tpupload.php?bh=".$bh);
  }
 }
...
document.getElementById("uping").style.display="";
location.href="tpupload.php?tpbh=<?=$_GET[tpbh]?>&action=del&bh=<?=$bh?>&tpid=<?=$_GET[tpid]?>&clearid=<?=$_GET[clearid]?>";	
}
</script>
</head>
<body>
...
<!--等待上传开始-->
<form method="post" name="tpf" enctype="multipart/form-data" action="tpupload.php?tpbh=<?=$_GET[tpbh]?>&action=update&bh=<?=$bh?>">

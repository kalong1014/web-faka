<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:95:"/www/wwwroot/www.811192.xyz/application/templates/pc/merchant/default/goods_category/index.html";i:1646323578;s:79:"/www/wwwroot/www.811192.xyz/application/templates/pc/merchant/default/base.html";i:1646323578;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
    <head>
        <meta charset="utf-8">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col-sm-12">
                                <button  onclick="$.x_show('添加分类', '<?php echo url('goods_category/add'); ?>', 450)" class="btn btn-primary glow invoice-create" role="button" aria-pressed="true"><i class="bx bx-plus mr-1"></i>添加分类</button>
                            </div>
                        </div>
                            ...
                            <table class="table mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><small class="text-muted"><?php echo date("Y-m-d H:i:s",$v['create_at']); ?></small></td>
                                        <td>

                                            <div class="btn-group btn-group-sm " role="group" aria-label="Basic group">
                                                <button  onclick="$.x_show('分类链接', '<?php echo url('goods_category/link',['id'=>$v['id']]); ?>', 560)"  class="btn btn-light waves-effect waves-light text-primary">链接</button>
                                                <a href="<?php echo url('goods/index',['cate_id'=>$v['id']]); ?>" class="btn btn-light waves-effect waves-light text-primary">商品</a>
                                                <a href="<?php echo url('goods_card/index',['cate_id'=>$v['id']]); ?>" class="btn btn-light waves-effect waves-light text-primary">库存卡</a>
                                                <button  onclick="$.x_show('编辑分类', '<?php echo url('goods_category/edit',['id'=>$v['id']]); ?>', 450)" href="" class="btn btn-light waves-effect waves-light text-primary">编辑</button>
                                                <a href="javascript:void(0);" onclick="del(this, '<?php echo $v['id']; ?>')" class="btn btn-light waves-effect waves-light text-danger">删除</a>
                                            </div>
...
<script>
    ...
    function change_status(obj, id) {
        ...
        $.post("<?php echo url('goods_category/changeStatus'); ?>", {
            id: id, status: status
        }, function (res) {
            if (res.code != 1) {
                $.alert(res.msg);
        ...
        $.confirm({
            ...
            buttons: {
                tryAgain: {
                    ...
                    action: function () {
                        var loading = layer.load(1, {shade: [0.1, '#fff']});
                        $.post("<?php echo url('goods_category/del'); ?>", {
                            id: id
                        }, function (res) {
                            layer.closeAll();
                            if (res.code != 1) {

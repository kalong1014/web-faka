...
function getPoint() {
	if($('#shoptid option:selected').val()=="0"){
		$('#inputsname').html("");
		$('#inputsname').hide();
		$('#alert_frame').hide();
		return false;
	}
	var multi = $('#shoptid option:selected').attr('multi');
	var count = $('#shoptid option:selected').attr('count');
	var price = $('#shoptid option:selected').attr('price');
	var shopimg = $('#shoptid option:selected').attr('shopimg');
	if(multi==1 && count>0){
	...
	var alert = $('#shoptid option:selected').attr('alert');
	if(alert!=''){
		layer.alert('<center>'+HotLove+'</center>',{
			btn:['我知道了'],
			title:'商品提示',
		});
	}else{
	...
	var inputname = $('#shoptid option:selected').attr('inputname');
	if(inputname!=''){
		$('#inputname').html(inputname);
	}else{
		$('#inputname').html("下单ＱＱ");
	}
	var inputsname = $('#shoptid option:selected').attr('inputsname');
	if(inputsname!=''){
		$('#inputsname').html("");
		$.each(inputsname.split('|'), function(i, value) {
			if(value.indexOf('{')>0 && value.indexOf('}')>0){
				...
				$.each(selectstr.split(','), function(i, v) {
					...
					addstr += '<option value="'+i+'" option>'+v+'</option>';
				});
	...
	if($('#shoptid option:selected').attr('isfaka')==1){
		$('#inputname').html("你的邮箱");
		$("#iffaka").html('自动发货');
		...
		$.ajax({
			...
			data : {tid:$('#shoptid option:selected').val()},
			dataType : 'json',
			success : function(data) {
...
function getshop(obj,tid,cid,name,price,input,inputs,multi,isfaka,value,alert,shopimg){
	$('#display_selectclass').hide();
	$('#usite').hide(); 
	$('#shoplist').hide(); 
	$('#shopinfo').show();
	$("#selected").html('填写商品信息 <select name="shoptid" id="shoptid"><option value="'+tid+'" cid="'+cid+'" price="'+price+'" inputname="'+input+'" inputsname="'+inputs+'" multi="'+multi+'" isfaka="'+isfaka+'" count="'+value+'" alert="'+escape(alert)+'" shopimg="'+shopimg+'">'+name+'</option></select><a class="btn btn-success btn-xs pull-right" href="../?cid='+cid+'">返回重选</a>');
	$("#infoshop").html('<div class="form-group text-center"><img src="'+shopimg+'" width="120" height="120" style="border-radius: 8px" id="shoptypes"><hr class="layui-bg-blue"><b>当前商品：'+generate(name)+' <br><hr class="layui-bg-orange">商品价格：<font color="red">￥'+price+'</font></b>');
	$("#imgshop").html('<img src="'+shopimg+'">');
	if(isfaka==1){
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data.code >= 0){
					$('#alert_frame').hide();
					alert('领取成功！');
					window.location.href='?mod=query';
				}else{
					layer.alert(data.msg);
					captchaObj.reset();
				}
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$.each(data.data, function (i, res) {
					$("#shoplist").append('<div class="col-xs-6 col-sm-3 col-md-3 layui-anim layui-anim-scaleSpring" data-anim="layui-anim-upbit"><a href="javascript:void(0)" id="'+res.tid+'" onclick="getshop(this,'+res.tid+',\''+cid+'\',\''+res.name+'\',\''+res.price+'\',\''+res.input+'\',\''+res.inputs+'\',\''+res.multi+'\',\''+res.isfaka+'\',\''+res.value+'\',\''+escape(res.alert)+'\',\''+res.shopimg+'\');" value="'+res.tid+'"><div class="thumbnail" style="height:240px;"><center style="margin-top:5%;"><img src="'+res.shopimg+'" width="80" height="100" style="border-radius: 15px"><hr class="layui-bg-blue" style="width:100%">'+res.name+'<hr class="layui-bg-red" style="width:100%">[￥'+res.price+']<br><span class="layui-badge layui-bg-blue">正常下单</span></center></div></a></div>');
					num++;
				});
				$("#shoplist").val(0);
				/*getPoint();*/
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				var num = 0;
				$.each(data.data, function (i, res) {
					$("#shoplist").append('<div class="col-xs-6 col-sm-3 col-md-3 layui-anim layui-anim-scaleSpring" data-anim="layui-anim-upbit"><a href="javascript:void(0)" id="'+res.tid+'" onclick="getshop(this,'+res.tid+',\''+cid+'\',\''+res.name+'\',\''+res.price+'\',\''+res.input+'\',\''+res.inputs+'\',\''+res.multi+'\',\''+res.isfaka+'\',\''+res.value+'\',\''+escape(res.alert)+'\',\''+res.shopimg+'\');" value="'+res.tid+'"><div class="thumbnail" style="height:240px;"><center style="margin-top:5%;"><img src="'+res.shopimg+'" width="80" height="100" style="border-radius: 15px"><hr class="layui-bg-blue" style="width:100%">'+generate(res.name)+'<hr class="layui-bg-red" style="width:100%">[￥'+res.price+']<br><span class="layui-btn layui-btn-xs layui-btn-radius layui-btn-normal">立即购买</span></center></div></a></div>');
					num++;
				});
				if($_GET["tid"]){
...
$("#num_min").click(function (){
	...
	if (i <= 0) {
		$("#num").val(1);
		i = 1;
		if(count>0)$('#need').val('￥'+$('#shoptid option:selected').attr('price') +"元 ➠ "+count+"个");
		else $('#need').val('￥'+$('#shoptid option:selected').attr('price') +"元");
	}
});
$("#num").blur(function () {
	var price = $('#shoptid option:selected').attr('price');
				...
				$.ajax({
					...
					success:function(data){
						if(data.code == 0){
							...
							layer.alert('恭喜您抽到奖品：'+data.name+'，即将跳转到商品所在分组!商品价格默认变成相应购买价格。', {
...
function ShowGG(id) {
	$.ajax({
		type : "GET",
		url : "ajax.php?act=getgonggao&id="+id,
		dataType : 'json',
		async: true,
		success : function(gg) {
			$('#ggtitle').html(gg.title);
			$('#ggbody').html(gg.body);
			$('#ggdate').html(gg.date);
			$('#gglook').html(gg.look);
			$('#Showgg').modal('show');

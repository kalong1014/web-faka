...
case 'add_gonggao':  //新增公告
	$title=$_POST['title'];
	$body=$_POST['body'];
	if(!$title||!$body){
		exit('{"code":-1,"msg":"请输入完整！"}');
	}
	$sql=$DB->query("INSERT INTO `shua_gonggao`(`title`,`body`) VALUES ('{$title}','{$body}')");
	if($sql){
		exit('{"code":0,"msg":"添加成功"}');
	}else{
		exit('{"code":1,"msg":"添加失败，'.$DB->error().'"}');
...
break;
case 'edit_gonggao':  //编辑公告
	$id=$_POST['id'];
	if(!$id){
		exit('{"code":-1,"msg":"请输入完整！"}');
	}
	$sql=$DB->get_row("SELECT * FROM shua_gonggao where id='{$id}'");
	if($sql){
		exit('{"code":0,"msg":"查询成功","id":"'.$id.'","title":"'.$sql['title'].'","body":"'.$sql['body'].'"}');
	}else{
		exit('{"code":1,"msg":"查询失败，'.$DB->error().'"}');
	}
break;
case 'del_gonggao':  //删除公告
	$id=$_POST['id'];
	if(!$id){
		exit('{"code":-1,"msg":"请输入完整！"}');
	}
	$sql=$DB->query("DELETE FROM shua_gonggao WHERE id='{$id}'");
	if($sql){
		exit('{"code":0,"msg":"删除成功"}');
	}else{
		exit('{"code":1,"msg":"删除失败，'.$DB->error().'"}');
...
case 'edit_gg_ok': //修改操作
	...
	$sql=$DB->query("UPDATE shua_gonggao set title='{$title}',body='{$body}' where id='{$id}'");
	if($sql){
		exit('{"code":0,"msg":"修改成功"}');
	}else{
		exit('{"code":1,"msg":"修改失败，'.$DB->error().'"}');

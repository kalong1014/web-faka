...
case 'getgonggao':
	$id=intval($_GET['id']);
	$title = $DB->count("select title from shua_gonggao where id='$id'");
	$body = $DB->count("select body from shua_gonggao where id='$id'");
	$date = $DB->count("select date from shua_gonggao where id='$id'");
	$look = $DB->count("select look from shua_gonggao where id='$id'");
	$looksun = $DB->count("UPDATE shua_gonggao SET look=look+1 WHERE id='$id'");
	$result=array("code"=>0,"id"=>$id,"title"=>$title,"body"=>$body,"date"=>$date,"look"=>$look);
